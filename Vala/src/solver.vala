        ...
        for (int i = 0; i < number_equations && i < number_variables; ++i)
        {
            ...
            else
            {
                ...
                for (int j = i + 1; j < number_variables; ++j)
                {
                    ...
                    {
                        solution_general[solution_indexes[i]] = "%s - x%d".printf (solution_general[solution_indexes[i]],
                                                                (solution_indexes[j] + 1));
                    }
                    else if (!matrix[i, j].equals (ZERO))
                    {
                        solution_general[solution_indexes[i]] = "%s - x%d * (%s)".printf (
                                solution_general[solution_indexes[i]], (solution_indexes[j] + 1), matrix[i, j].to_string());
                    }
                }
            }
        }
